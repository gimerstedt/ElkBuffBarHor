...
function ElkBuffBar:InitSettings()
		...
		if not ElkBuffBarOptions.height then ElkBuffBarOptions.height = 40 end
		if not ElkBuffBarOptions.scale then ElkBuffBarOptions.scale = 1 end
		if not ElkBuffBarOptions.anchor then ElkBuffBarOptions.anchor = "BOTTOMRIGHT" end
		...
		if not ElkBuffBarOptions.sort then ElkBuffBarOptions.sort = "TIMELEFTCATEGORY" end
...
-- (id, buffname, timeleft, timemax, cancel, category)
local function buffssort_category(a, b)
	if (not a or not b) then
		return true
	end
	if a[6] > b[6] then
...
function ElkBuffBar:UpdateBuffs()
	...
	while true do
		...
		local found = false
		for buffCategory, buffs in pairs(ElkBuffBarData) do
			if found == true then break end
			for key, buffName in pairs(buffs) do
				if found == true then break end
				if buffName == name then found = true category = buffCategory end
			end
		end
		if found == false then category = 9 end
		
		table.insert(buffssort, self.compost:Acquire(buffIndex, name, timeleft, timemax, untilCancelled, category))
		i = i + 1
	end
	...
	elseif ElkBuffBarOptions.sort == "TIMELEFTCATEGORY" then
		table.sort(buffssort, buffssort_category)
	end
	for it = 1, table.getn(buffssort) do
		numBuffs = numBuffs + 1
		self:SetBuffButtonBuff(numBuffs, buffssort[it][1], buffssort[it][2], buffssort[it][3], buffssort[it][4], buffssort[it][5], buffssort[it][6])
	end
...
function ElkBuffBar:SetBuffButtonBuff(buttonid, id, buffname, timeleft, timemax, cancel, category)
...
function ElkBuffBar:OnUpdate(elapsed)
	...
	for i = 1, self.numBuffsShown do
		...
		if button.untilCancelled ~= 1 then
			local timeleft = 0
			if button.type == "DEBUFF" or type(button.type) == "number" then
...
function ElkBuffBar:UpdateLayout()
	...
	if ElkBuffBarOptions.x and ElkBuffBarOptions.y and ((not self.Layout.x or self.Layout.x ~= ElkBuffBarOptions.x) or (not self.Layout.y or self.Layout.y ~= ElkBuffBarOptions.y) or (not self.Layout.anchor or self.Layout.anchor ~= ElkBuffBarOptions.anchor) or (not self.Layout.invert or self.Layout.invert ~= ElkBuffBarOptions.invert)) then
		...
		if ElkBuffBarOptions.invert then
			ElkBuffBarOptions.anchor = "BOTTOMLEFT" -- bottomleft
		else
			ElkBuffBarOptions.anchor = "BOTTOMRIGHT" -- bottomright
		end
...
function ElkBuffBar:UpdateButtonPosition(buttonid, space)
	...
	button:SetPoint("TOP", self.frame, "TOP") --left
	button:SetPoint("BOTTOM", self.frame, "BOTTOM") --right
	if buttonid > 1 then
		if (ElkBuffBarOptions.invert) then
		...
		if (ElkBuffBarOptions.invert) then
			button:SetPoint("TOPLEFT", self.frame, "TOPLEFT") --bottom/bottom
		else
			button:SetPoint("TOPRIGHT", self.frame, "TOPRIGHT") --top/top
		end
	end
end

function ElkBuffBar:UpdateButtonPositions(onlywidth)
	if self.numBuffsShown > 0 then
		local groupspacer = 0
		local groupspacer2 = 0
		local lasttype
		for i = 1, self.numBuffsShown do
			local button = getglobal("ElkBuffButton"..i)
			if lasttype and button.type ~= lasttype then
				if not onlywidth then
					if type(button.type) ~= "number" then
						self:UpdateButtonPosition(i, ElkBuffBarOptions.spacing + ElkBuffBarOptions.group*5)
						groupspacer2 = groupspacer2 + 1
					else
		...
		local buffsWidth = self.numBuffsShown * ElkBuffBarOptions.width
		local buffSpacing = (self.numBuffsShown - 1) * ElkBuffBarOptions.spacing
		local buffCategorySpacing = groupspacer * ElkBuffBarOptions.group
		local buffGroupSpacing = groupspacer2 * ElkBuffBarOptions.group * 5
		local w =  buffsWidth + buffSpacing + buffCategorySpacing + buffGroupSpacing	
		
		self.frame:SetWidth(w)
		self.frame:SetHeight(14+ElkBuffBarOptions.height);
	else
...
-- print("|cffffff00[ElkBuffBar]|r msg is >|cffff0000"..(msg or "NIL").."|r<, carg1 is >|cffff0000"..(carg1 or "NIL").."|r<, carg2 is >|cffff0000"..(carg2 or "NIL").."|r<")
	...
	elseif carg1 == "sort" then
		if carg2 then
			carg2 = string.upper(carg2)
			if carg2 ~= "DEFAULT" and carg2 ~= "TIMEMAX" and carg2 ~= "TIMELEFT" and carg2 ~= "TIMELEFTCATEGORY" then
				DEFAULT_CHAT_FRAME:AddMessage("|cffffff00[ElkBuffBar]|r SORT must be DEFAULT, TIMEMAX, TIMELEFT or TIMELEFTCATEGORY")
			else
